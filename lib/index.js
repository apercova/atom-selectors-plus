 ...
 * <a href="https://www.npmjs.com/~apercova" target="_blank">https://www.npmjs.com/~apercova</a>
 ...
 */
function getCache() {
  return cachedMatchesBySelector;
}
...
function getCachedMatch(selector, scopeChain, ns) {
  const cachedMatchesByScopeChain = (cachedMatchesBySelector[ns || 'root'] ||
    {})[selector];
  return cachedMatchesByScopeChain
...
function isSubset(subset, superset) {
  return (
    ...
    subset.filter(sub => superset.includes(sub)).length === subset.length
  );
}
...
function selectorMatchesAnyScope(selector, scopes, useCache) {
  ...
  if (useCache === false) {
    ...
    return selectorTokens.some(_selector =>
      scopes.some(matcherForSelector(_selector))
    );
  } else {
    let scopeChain =
      typeof scopes === 'string'
        ? scopes
        : Array.isArray(scopes)
        ? buildScopeChain(scopes)
        : '';

    const cachedMatch = getCachedMatch(selector, scopeChain, matchAnyCacheNS);
    if (typeof cachedMatch === 'boolean') {
      return cachedMatch;
    } else {
      const selectorTokens = selector.trim().split(/[\t\s]+/);
      const match = selectorTokens.some(_selector =>
        scopes.some(matcherForSelector(_selector))
      );
...
function anySelectorMatchAnyScope(selectors, scopes, useCache) {
  selectors =
    typeof selectors === 'string'
      ...
      ? selectors
      : [];
  return selectors.some(_selector =>
    selectorMatchesAnyScope(_selector, scopes, useCache)
  );
}
...
function selectorMatchesAllScopes(selector, scopes, useCache) {
  ...
  if (useCache === false) {
    ...
    const match = selectorTokens.every(selector =>
      selectorMatchesAnyScope(selector, scopes, useCache)
    );
    return match;
  } else {
    let scopeChain =
...
function anySelectorMatchAllScopes(selectors, scopes, useCache) {
  selectors =
    typeof selectors === 'string'
      ...
      : [];
  return selectors.some(_selector =>
    selectorMatchesAllScopes(_selector, scopes, useCache)
  );
}
